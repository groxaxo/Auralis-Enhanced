...
try:
    ...
    logger.error(
        f"Failed to load model: {e}. Ensure that the model exists at {model_path}"
    )
    ...
    with gr.Blocks(title="Auralis TTS Demo", theme="soft") as ui:
        ...
        with gr.Tab("File to Speech"):
            with gr.Row():
                with gr.Column():
                    file_input = gr.File(
                        label="Text / Ebook File", file_types=["text", ".epub"]
                    )
...
if __name__ == "__main__":
    ui = build_gradio_ui()
    ui.launch(debug=True, server_name="0.0.0.0", server_port=7863, share=False)
